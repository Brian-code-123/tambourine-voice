...
def _assert_no_visible_checklist(prompt: str) -> None:
    ...
    """
    s = prompt.lower()
    patterns = [r"\bbegin\b.*\bchecklist\b", r"\boutput\b.*\bchecklist\b", r"\bchecklist\b.*\boutput\b"]
    for p in patterns:
        assert not re.search(p, s), f"Prompt appears to ask for visible checklist (matched pattern: {p})"
